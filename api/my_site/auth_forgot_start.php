...
function zb_forgot_bd_local(string $phone): string {
    $d = preg_replace('/\D+/', '', trim($phone)) ?? '';
    if (str_starts_with($d, '880')) { $d = '0' . substr($d, 3); }
    if (strlen($d) === 10 && str_starts_with($d, '1')) { $d = '0' . $d; }
    return $d;
}
function zb_forgot_valid_bd(string $phone): bool { return preg_match('/^01[3-9]\d{8}$/', zb_forgot_bd_local($phone)) === 1; }

$body = api_read_json_body();
$phoneLocal = zb_forgot_bd_local((string)($body['phone'] ?? $body['number'] ?? ''));
$phoneE164 = $phoneLocal !== '' ? '88' . $phoneLocal : '';
if (!zb_forgot_valid_bd($phoneLocal)) { api_response(false, 'INVALID_BD_NUMBER', 'Valid Bangladesh number is required', [], 422); }
...
$now = time();
$forgotToken = 'ZBFORGOT_' . random_token(24);
$forgotHash = zb_token_hash($forgotToken);
$otpId = 'ZBFORGOTOTP_' . date('YmdHis') . strtoupper(substr(bin2hex(random_bytes(3)), 0, 6));
...
fb_put('Z_BUILDER_OWNER_FORGOT/' . $forgotHash, $forgotRow);
fb_put('Z_BUILDER_OWNER_OTPS/' . $otpId, $otpRow);
$message = 'Z Builder reset OTP: ' . $otp . '. Valid for 5 minutes.';
