...
$reason = znews_comment_moderation_note(
...
if (empty($result['ok'])) {
    api_response(
        false,
        (string)($result['code'] ?? 'ZNEWS_COMMENT_REJECT_FAILED'),
        (string)($result['message'] ?? 'Comment could not be rejected.'),
        array_filter([
...
api_response(true, 'ZNEWS_COMMENT_REJECTED', 'Comment rejected.', [
