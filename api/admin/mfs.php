...
<head>
  ...
  <link rel="stylesheet" href="assets/mfs-panel.css?v=1">
  <style>
...
<body>
  ...
  (function(){
    'use strict';
    var state = { tab:'pending', rows:[], csrf:'' };
    ...
    async function markSuccess(id){ await ensureCsrf(); var digit=prompt('Sender number last digit required. Example: 5',''); if(digit===null)return; digit=String(digit).trim(); if(!/^\d$/.test(digit)){ alert('Only one last digit required. Example: 5'); return; } var trxid=prompt('TRXID optional. Leave empty if not available.',''); if(trxid===null)return; try{ await post('mfs_success',{request_id:id,trxid:trxid||'',sender_last_digit:digit,message:'Transaction successful. Sender last digit: '+digit}); msg('Marked successful: '+id,'good'); await load(); }catch(e){ msg(e.message||'Failed to mark success','bad'); } }
    ...
    function bind(){ document.querySelectorAll('.admin-mfs-tab').forEach(function(btn){btn.addEventListener('click',function(){document.querySelectorAll('.admin-mfs-tab').forEach(function(b){b.classList.remove('active')}); btn.classList.add('active'); state.tab=btn.getAttribute('data-mfs-tab')||'pending'; load();});}); el('mfsReloadBtn').addEventListener('click',load); el('mfsApplyFilterBtn').addEventListener('click',load); el('mfsSearch').addEventListener('input',render); el('mfsTableBody').addEventListener('click',onAction); el('mfsMobileList').addEventListener('click',onAction); }
