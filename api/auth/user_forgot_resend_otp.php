...
if (!is_array($oldOtpRow)) {
    user_forgot_res_response(false, 'OTP_NOT_FOUND', 'Previous OTP request not found', [], 404);
}

$resendState = auth_otp_resend_state($oldOtpRow, $now);
if (empty($resendState['ok'])) {
    user_forgot_res_response(false, (string)$resendState['code'], (string)$resendState['message'], [
