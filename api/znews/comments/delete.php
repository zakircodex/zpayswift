...
if (empty($result['ok'])) {
    api_response(
        false,
        (string)($result['code'] ?? 'ZNEWS_COMMENT_DELETE_FAILED'),
        (string)($result['message'] ?? 'Comment could not be deleted.'),
        array_filter([
...
api_response(true, 'ZNEWS_COMMENT_DELETED', 'Comment deleted.', [
