<?php
/**
 * Telegram webhook for MFS requests.
 *
 * Button behavior:
...
function mfs_tg_now(): int { return function_exists('now_ts') ? (int)now_ts() : time(); }
function mfs_tg_token(): string { return defined('TELEGRAM_BOT_TOKEN') ? trim((string)TELEGRAM_BOT_TOKEN) : ''; }
function mfs_tg_chat_id(): string { return defined('TELEGRAM_CHAT_ID') ? trim((string)TELEGRAM_CHAT_ID) : ''; }
function mfs_tg_secret(): string { return defined('TELEGRAM_WEBHOOK_SECRET') ? trim((string)TELEGRAM_WEBHOOK_SECRET) : ''; }
function mfs_tg_h($v): string { return htmlspecialchars((string)$v, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); }
function mfs_tg_money($v): string { return number_format((float)$v, 2, '.', ''); }

function mfs_tg_require_config(): void
{
    $missing = [];
    if (mfs_tg_token() === '') $missing[] = 'TELEGRAM_BOT_TOKEN';
    if (mfs_tg_chat_id() === '') $missing[] = 'TELEGRAM_CHAT_ID';
    if (mfs_tg_secret() === '') $missing[] = 'TELEGRAM_WEBHOOK_SECRET';
    if ($missing) mfs_tg_json(false, 'CONFIG_ERROR', 'Telegram config missing', ['missing' => $missing], 500);
}
...
function mfs_tg_verify_secret(): void
{
    ...
    if ($secret === '') mfs_tg_json(false, 'CONFIG_ERROR', 'TELEGRAM_WEBHOOK_SECRET missing', [], 500);
    if ($query !== '' && hash_equals($secret, $query)) return;
...
function mfs_tg_api(string $method, array $payload): array
{
    ...
    return ['ok' => $http >= 200 && $http < 300 && is_array($json) && !empty($json['ok']), 'http' => $http, 'json' => is_array($json) ? $json : [], 'error' => $err, 'raw' => is_string($raw) ? substr($raw, 0, 500) : ''];
}

function mfs_tg_answer(string $callbackId, string $text, bool $alert = false): void
{
    if ($callbackId === '') return;
    mfs_tg_api('answerCallbackQuery', ['callback_query_id' => $callbackId, 'text' => $text, 'show_alert' => $alert]);
}

function mfs_tg_send($chatId, string $text, array $extra = []): void
{
    if ((string)$chatId === '') return;
    mfs_tg_api('sendMessage', array_merge(['chat_id' => $chatId, 'text' => $text, 'parse_mode' => 'HTML', 'disable_web_page_preview' => true], $extra));
}

function mfs_tg_edit($chatId, $messageId, string $text, array $replyMarkup = []): void
{
    if ((string)$chatId === '' || (string)$messageId === '') return;
    mfs_tg_api('editMessageText', ['chat_id' => $chatId, 'message_id' => $messageId, 'text' => $text, 'parse_mode' => 'HTML', 'disable_web_page_preview' => true, 'reply_markup' => $replyMarkup ?: ['inline_keyboard' => []]]);
}
...
function mfs_tg_allowed($chatId, $fromId = ''): bool
{
    ...
    return $allowed !== '' && (($chatId !== '' && hash_equals($allowed, $chatId)) || ($fromId !== '' && hash_equals($allowed, $fromId)));
}

function mfs_tg_parse_action(string $data): array
{
    $parts = explode('|', trim($data));
    if (count($parts) !== 2) return ['ok' => false, 'message' => 'Invalid callback data'];
    $map = ['MFS_PROCESSING' => 'PROCESSING', 'MFS_SUCCESS' => 'SUCCESS', 'MFS_FAILED' => 'FAILED'];
    $action = $map[strtoupper(trim($parts[0]))] ?? '';
    $requestId = trim($parts[1]);
    if ($action === '') return ['ok' => false, 'message' => 'Invalid MFS action'];
    if (!preg_match('/^[A-Za-z0-9_-]{3,120}$/', $requestId)) return ['ok' => false, 'message' => 'Invalid request id'];
    return ['ok' => true, 'action' => $action, 'request_id' => $requestId];
}

function mfs_tg_actor(array $from): array
{
    return ['uid' => 'TELEGRAM:' . (string)($from['id'] ?? ''), 'role' => 'TELEGRAM_ADMIN'];
}
...
{
    fb_put(mfs_tg_pending_path($fromId), ['request_id' => $requestId, 'chat_id' => (string)$chatId, 'message_id' => (string)$messageId, 'status' => 'WAITING_LAST_DIGIT', 'created_at' => mfs_tg_now(), 'expires_at' => mfs_tg_now() + 600]);
}
...
{
    $text = trim($text);
    if (preg_match('/^\d$/', $text)) return $text;
    if (preg_match('/(\d)\s*$/', $text, $m)) return (string)$m[1];
    return '';
}
...
function mfs_tg_save_digit(string $requestId, string $digit): void
{
    ...
    if (!in_array($bucket, ['PENDING', 'PROCESSING'], true)) return;
    fb_patch('MFS_REQUESTS/' . $bucket . '/' . $requestId, ['sender_last_digit' => $digit, 'sender_last_number_digit' => $digit, 'last_digit' => $digit, 'updated_at' => mfs_tg_now()]);
}
...
function mfs_tg_handle_callback(array $callback): void
{
    ...
    if ($action === 'PROCESSING') {
        ...
        if (empty($res['ok'])) mfs_tg_json(true, (string)($res['code'] ?? 'FAILED'), (string)($res['message'] ?? 'Failed'), ['result' => $res], 200);
        $updated = mfs_find_request($requestId) ?: $row;
        mfs_tg_edit($chatId, $messageId, mfs_tg_text($updated, 'PROCESSING', 'MFS request is processing'));
    ...
    if ($action === 'SUCCESS') {
        ...
        mfs_tg_edit($chatId, $messageId, mfs_tg_text($row, 'WAITING_LAST_DIGIT', 'Sender number last digit required'), ['inline_keyboard' => [[['text' => 'Failed', 'callback_data' => 'MFS_FAILED|' . $requestId]]]]);
        mfs_tg_send($chatId, '🔢 <b>Sender number last digit দিন</b>\n\nRequest ID: <code>' . mfs_tg_h($requestId) . '</code>\nExample: <code>5</code>\n\nLast digit না দিলে success হবে না।', ['reply_markup' => ['force_reply' => true, 'input_field_placeholder' => 'Sender last digit']]);
...
function mfs_tg_handle_message(array $message): void
{
    ...
    if (!mfs_tg_allowed($chatId, $fromId)) mfs_tg_json(true, 'IGNORED', 'Unauthorized Telegram message', [], 200);
    $pending = mfs_tg_get_pending($fromId);
    if (!$pending) mfs_tg_json(true, 'IGNORED', 'No pending MFS success confirmation', [], 200);
    ...
    mfs_tg_clear_pending($fromId);
    $done = mfs_find_request($requestId) ?: ['request_id' => $requestId, 'sender_last_digit' => $digit];
    ...
    mfs_tg_json(true, 'SUCCESS', 'MFS request marked successful', ['request_id' => $requestId, 'sender_last_digit' => $digit], 200);
}

mfs_tg_require_config();
mfs_tg_verify_secret();
if (strtoupper($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') mfs_tg_json(true, 'OK', 'Telegram MFS webhook is ready', ['time' => date('Y-m-d H:i:s')], 200);

$raw = file_get_contents('php://input');
$update = is_string($raw) && trim($raw) !== '' ? json_decode($raw, true) : null;
if (!is_array($update)) mfs_tg_json(true, 'IGNORED', 'Invalid or empty Telegram update', [], 200);
if (isset($update['callback_query']) && is_array($update['callback_query'])) mfs_tg_handle_callback($update['callback_query']);
if (isset($update['message']) && is_array($update['message'])) mfs_tg_handle_message($update['message']);
