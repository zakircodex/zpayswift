...
edit_composer_expect(str_contains($creator, "URL.revokeObjectURL(editPreviewUrl)"), 'Replacement-photo object URLs are not released.');
edit_composer_expect(str_contains($creator, "['image/jpeg', 'image/png', 'image/webp']"), 'Edit photo MIME allowlist is missing.');
edit_composer_expect(str_contains($creator, 'replacement.size > 5 * 1024 * 1024'), 'Edit photo size limit is missing.');
...
edit_composer_expect(str_contains($index, 'znews-bootstrap.js?v=23'), 'Reload-safe edit bootstrap is not activated.');

foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    edit_composer_expect(str_contains($worker, 'znews-bootstrap.js?v=23'), 'A PWA shell is missing the reload-safe edit bootstrap.');
