...
znews_contract_expect(str_contains($index, 'znews-bootstrap.js?v=38'), 'Reload-safe handoff bootstrap must be activated.');
znews_contract_expect(str_contains($bootstrap, 'znews-weekly-review.js?v=4'), 'Creator weekly report module must remain available after verified access.');
...
znews_contract_expect(str_contains($api, 'timeoutMs: 15000'), 'Stored creator validation must allow the bounded backend auth path to complete.');
...
znews_contract_expect(str_contains($bootstrap, 'SESSION_VALIDATION_MAX_ATTEMPTS = 2'), 'Stored creator validation must retry at most once.');
znews_contract_expect(str_contains($bootstrap, 'sessionValidationIsTerminal(error)'), 'Terminal auth failures must not be retried.');
...
znews_contract_expect(str_contains($bootstrap, 'znews-reader.js?v=5'), 'Latest post reader module must load after public content.');
...
znews_contract_expect(str_contains($access, "['create', 'mine', 'performance']"), 'Guest-only route guard must cover creator sections.');
...
znews_contract_expect(str_contains($serviceWorker, 'zsky24-embedded-shell-v37'), 'Current embedded PWA cache namespace is missing.');
znews_contract_expect(str_contains($serviceWorker, 'znews-reader.js?v=5'), 'Latest reader module must be cached.');
znews_contract_expect(str_contains($serviceWorker, 'znews-weekly-review.js?v=4'), 'Creator weekly report module must be cached.');
