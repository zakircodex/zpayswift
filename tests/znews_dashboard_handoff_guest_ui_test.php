...
znews_contract_expect(str_contains($index, 'znews-bootstrap.js?v=5'), 'Latest handoff bootstrap must be activated.');
...
znews_contract_expect(str_contains($bootstrap, 'znews-reader.js?v=1'), 'Post reader module must load after creator access is resolved.');
...
znews_contract_expect(str_contains($serviceWorker, 'znews-shell-v8'), 'PWA cache must include the latest reader/fair-feed/access model.');
znews_contract_expect(str_contains($serviceWorker, 'znews-reader.js?v=1'), 'Reader module must be cached.');
