...
instant_expect(str_contains($createService, 'znews_post_publication_decision($mediaRow, $moderationText)')
    && str_contains($createService, '$title . "\n" . $text'), 'create service does not check the headline and post text');
instant_expect(str_contains($createService, 'znews_path_public_feed($postId)'), 'create service lacks public-feed write');
instant_expect(str_contains($updateService, 'znews_post_publication_decision($newMediaRow, $moderationText)')
    && str_contains($updateService, '$targetTitle . "\n" . $text'), 'edited headline and text are not rechecked');
...
instant_expect(str_contains($webBootstrap, 'znews-creator.js?v=15'), 'web creator management module is not loaded by bootstrap');
