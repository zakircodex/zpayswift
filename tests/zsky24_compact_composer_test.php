...
composer_expect(str_contains($index, 'znews-premium.css?v=10'), 'Composer stylesheet cachebuster is missing.');
composer_expect(str_contains($index, 'znews-bootstrap.js?v=22'), 'Reload-safe composer bootstrap cachebuster is missing.');
composer_expect(str_contains($bootstrap, 'znews.js?v=20'), 'Latest composer behavior is not loaded.');
...
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    composer_expect(str_contains($worker, 'znews-premium.css?v=10'), 'Latest composer stylesheet is missing from a PWA shell.');
    composer_expect(str_contains($worker, 'znews-bootstrap.js?v=22'), 'Latest reload-safe bootstrap is missing from a PWA shell.');
    composer_expect(str_contains($worker, 'znews.js?v=20'), 'Latest app behavior is missing from a PWA shell.');
