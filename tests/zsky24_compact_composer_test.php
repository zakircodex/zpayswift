...
composer_expect(str_contains($index, 'znews-bootstrap.js?v=12'), 'Composer bootstrap cachebuster is missing.');
composer_expect(str_contains($bootstrap, 'znews.js?v=10'), 'Latest composer behavior is not loaded.');
composer_expect(str_contains($bootstrap, 'znews-creator.js?v=6'), 'Latest creator behavior is not loaded.');
...
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    ...
    composer_expect(str_contains($worker, 'znews-bootstrap.js?v=12'), 'Latest bootstrap is missing from a PWA shell.');
    composer_expect(str_contains($worker, 'znews.js?v=10'), 'Latest app behavior is missing from a PWA shell.');
    composer_expect(str_contains($worker, 'znews-creator.js?v=6'), 'Latest creator behavior is missing from a PWA shell.');
