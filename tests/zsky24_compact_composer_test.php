...
composer_expect(str_contains($index, 'znews-bootstrap.js?v=34'), 'Reload-safe composer bootstrap cachebuster is missing.');
composer_expect(str_contains($index, 'znews.js?v=28'), 'Latest composer behavior is not loaded.');
...
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    ...
    composer_expect(str_contains($worker, 'znews-bootstrap.js?v=34'), 'Latest reload-safe bootstrap is missing from a PWA shell.');
    composer_expect(str_contains($worker, 'znews.js?v=28'), 'Latest app behavior is missing from a PWA shell.');
    ...
    composer_expect(str_contains($worker, "SHELL_REVISION = 'weekly-performance-mobile-load-1'"), 'Weekly performance cache revision is missing from a PWA shell.');
