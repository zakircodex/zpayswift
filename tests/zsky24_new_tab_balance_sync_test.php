...
new_tab_balance_expect(str_contains($index, 'znews-bootstrap.js?v=32'), 'Feed document does not bypass the stale bootstrap cache.');
new_tab_balance_expect(str_contains($index, 'znews.js?v=27'), 'Document does not bypass the stale feed app cache.');
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    new_tab_balance_expect(str_contains($worker, 'znews-bootstrap.js?v=32'), 'A PWA shell is missing the refreshed bootstrap URL.');
    new_tab_balance_expect(str_contains($worker, 'znews.js?v=27'), 'A PWA shell is missing the refreshed feed app URL.');
}
