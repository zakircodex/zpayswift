...
new_tab_balance_expect(str_contains($index, 'znews-bootstrap.js?v=22'), 'Feed document does not bypass the stale bootstrap cache.');
new_tab_balance_expect(str_contains($bootstrap, 'znews.js?v=20'), 'Bootstrap does not bypass the stale feed app cache.');
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    new_tab_balance_expect(str_contains($worker, 'znews-bootstrap.js?v=22'), 'A PWA shell is missing the refreshed bootstrap URL.');
    new_tab_balance_expect(str_contains($worker, 'znews.js?v=20'), 'A PWA shell is missing the refreshed feed app URL.');
}
