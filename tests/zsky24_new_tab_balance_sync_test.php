...
new_tab_balance_expect(str_contains($index, 'znews-bootstrap.js?v=31'), 'Feed document does not bypass the stale bootstrap cache.');
new_tab_balance_expect(str_contains($index, 'znews.js?v=26'), 'Document does not bypass the stale feed app cache.');
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    new_tab_balance_expect(str_contains($worker, 'znews-bootstrap.js?v=31'), 'A PWA shell is missing the refreshed bootstrap URL.');
    new_tab_balance_expect(str_contains($worker, 'znews.js?v=26'), 'A PWA shell is missing the refreshed feed app URL.');
}
