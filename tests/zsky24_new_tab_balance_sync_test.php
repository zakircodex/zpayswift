...
new_tab_balance_expect(str_contains($index, 'znews.js?v=32'), 'Document does not bypass the stale feed app cache.');
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    ...
    new_tab_balance_expect(str_contains($worker, 'znews.js?v=32'), 'A PWA shell is missing the refreshed feed app URL.');
}
