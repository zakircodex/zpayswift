...
new_tab_balance_expect(str_contains($index, 'znews-bootstrap.js?v=30'), 'Feed document does not bypass the stale bootstrap cache.');
...
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    new_tab_balance_expect(str_contains($worker, 'znews-bootstrap.js?v=30'), 'A PWA shell is missing the refreshed bootstrap URL.');
