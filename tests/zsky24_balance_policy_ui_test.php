...
balance_policy_expect(str_contains($index, 'znews.js?v=31'), 'Current feed application cache version is not active.');

foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    balance_policy_expect(str_contains($worker, 'znews.js?v=31'), 'A PWA shell is missing the current application asset.');
