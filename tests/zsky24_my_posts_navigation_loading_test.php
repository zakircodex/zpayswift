...
my_posts_expect(str_contains($bootstrap, 'znews.js?v=16'), 'Latest navigation behavior is not activated.');
my_posts_expect(str_contains($bootstrap, 'znews-creator.js?v=7'), 'Latest creator modal behavior is not activated.');
my_posts_expect(str_contains($embeddedWorker, "zsky24-embedded-shell-v13"), 'Embedded cache namespace is stale.');
my_posts_expect(str_contains($standaloneWorker, "zsky24-standalone-shell-v13"), 'Standalone cache namespace is stale.');
my_posts_expect(str_contains($embeddedWorker, 'znews.js?v=16') && str_contains($embeddedWorker, 'znews-creator.js?v=7'), 'Embedded shell is missing the updated scripts.');
my_posts_expect(str_contains($standaloneWorker, 'znews.js?v=16') && str_contains($standaloneWorker, 'znews-creator.js?v=7'), 'Standalone shell is missing the updated scripts.');
