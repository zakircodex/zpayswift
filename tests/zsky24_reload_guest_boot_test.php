...
zsky24_reload_expect(str_contains($index, 'znews-config.js?v=9'), 'config cache-busting revision was not advanced');
zsky24_reload_expect(str_contains($index, 'znews-api.js?v=14'), 'API cache-busting revision was not advanced');
zsky24_reload_expect(str_contains($index, 'znews-bootstrap.js?v=33'), 'bootstrap cache-busting revision was not advanced');
zsky24_reload_expect(str_contains($bootstrap, 'znews-weekly-review.js?v=2'), 'weekly creator report cache-busting revision is missing');
...
zsky24_reload_expect(str_contains($api, 'timeoutMs: 15000'), 'stored creator session validation does not allow the bounded backend auth path to complete');
...
zsky24_reload_expect(str_contains($index, 'defer src="/znews/assets/znews.js?v=28"'), 'deferred app shell revision was not advanced');

foreach ([$standaloneSw, $embeddedSw] as $sw) {
    zsky24_reload_expect(str_contains($sw, 'shell-v28'), 'service worker cache generation was not advanced');
    ...
    zsky24_reload_expect(str_contains($sw, 'znews-config.js?v=9'), 'service worker config revision does not match index');
    zsky24_reload_expect(str_contains($sw, 'znews-api.js?v=14'), 'service worker API revision does not match index');
    zsky24_reload_expect(str_contains($sw, 'znews-weekly-review.js?v=2'), 'service worker weekly report revision does not match index');
    zsky24_reload_expect(str_contains($sw, 'znews-weekly-review.css?v=2'), 'service worker weekly report stylesheet does not match index');
    zsky24_reload_expect(str_contains($sw, 'znews-bootstrap.js?v=33'), 'service worker bootstrap revision does not match index');
    ...
    zsky24_reload_expect(str_contains($sw, 'znews.js?v=28'), 'service worker app revision does not match index');
