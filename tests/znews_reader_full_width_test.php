...
$expect(str_contains($sw, "const SHELL_REVISION = 'social-ui-replies-v1'"), 'Service worker revision must refresh current assets without regressing the reader.');
$expect(str_contains($sw, "const CACHE_NAME = 'zsky24-embedded-shell-v37'"), 'Embedded shell cache namespace is incorrect.');
$expect(str_contains($sw, 'znews-reader.css?v=4'), 'Reader stylesheet is missing from the current shell.');
