...
$expect(str_contains($sw, "const SHELL_REVISION = 'weekly-performance-premium-1'"), 'Service worker revision must refresh weekly UI assets without regressing the reader.');
$expect(str_contains($sw, "const CACHE_NAME = 'zsky24-embedded-shell-v28'"), 'Embedded shell cache namespace is incorrect.');
$expect(str_contains($sw, 'znews-reader.css?v=2'), 'Reader stylesheet is missing from the current shell.');
$expect(str_contains($sw, 'znews-weekly-review.js?v=2'), 'Weekly creator report asset is missing from the current shell.');
