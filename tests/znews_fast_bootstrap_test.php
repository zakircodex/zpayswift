...
fast_boot_expect(str_contains($bootstrap, "loadStylesheet('/znews/assets/znews-reader.css?v=3')"), 'Reader CSS post-paint loader is missing.');
fast_boot_expect(str_contains($bootstrap, "loadScript('/znews/assets/znews-creator.js?v=15')"), 'Creator module is unavailable after verified auth.');
...
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    fast_boot_expect(str_contains($worker, 'shell-v36'), 'Service-worker cache generation was not advanced.');
    fast_boot_expect(str_contains($worker, "SHELL_REVISION = 'categories-device-share-v1'"), 'Category/device shell revision is missing.');
    fast_boot_expect(str_contains($worker, 'znews-device-specs.js?v=1'), 'Service worker is missing the device catalog module.');
    fast_boot_expect(str_contains($worker, 'znews-rich-editor.js?v=6'), 'Service worker is missing the single-surface rich editor module.');
    fast_boot_expect(str_contains($worker, 'znews-bootstrap.js?v=37'), 'Service worker has a stale bootstrap URL.');
    fast_boot_expect(str_contains($worker, 'znews.js?v=33'), 'Service worker has a stale app URL.');
