...
fast_boot_expect(str_contains($bootstrap, "loadStylesheet('/znews/assets/znews-reader.css?v=4')"), 'Reader CSS post-paint loader is missing.');
...
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    fast_boot_expect(str_contains($worker, 'shell-v37'), 'Service-worker cache generation was not advanced.');
    fast_boot_expect(str_contains($worker, "SHELL_REVISION = 'social-ui-replies-v1'"), 'Social UI shell revision is missing.');
    ...
    fast_boot_expect(str_contains($worker, 'znews-bootstrap.js?v=38'), 'Service worker has a stale bootstrap URL.');
    fast_boot_expect(str_contains($worker, 'znews.js?v=34'), 'Service worker has a stale app URL.');
