...
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    fast_boot_expect(str_contains($worker, 'shell-v27'), 'Service-worker cache generation was not advanced.');
    fast_boot_expect(str_contains($worker, "SHELL_REVISION = 'rich-editor-single-surface-1'"), 'Single-surface editor shell revision is missing.');
    ...
    fast_boot_expect(str_contains($worker, 'znews-bootstrap.js?v=32'), 'Service worker has a stale bootstrap URL.');
    fast_boot_expect(str_contains($worker, 'znews.js?v=27'), 'Service worker has a stale app URL.');
