...
$critical = [
    ...
    'znews-ads.js?v=2',
    'znews-rich-editor.js?v=1',
    'znews-bootstrap.js?v=28',
    'znews.js?v=24',
];
$last = -1;
...
fast_boot_expect(str_contains($bootstrap, "loadScript('/znews/assets/znews-creator.js?v=10')"), 'Creator module is unavailable after verified auth.');
...
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    fast_boot_expect(str_contains($worker, 'shell-v23'), 'Service-worker cache generation was not advanced.');
    fast_boot_expect(str_contains($worker, "SHELL_REVISION = 'rich-editor-ui-1'"), 'Rich editor UI shell revision is missing.');
    fast_boot_expect(str_contains($worker, 'znews-rich-editor.js?v=1'), 'Service worker is missing the safe rich editor module.');
    fast_boot_expect(str_contains($worker, 'znews-bootstrap.js?v=28'), 'Service worker has a stale bootstrap URL.');
    fast_boot_expect(str_contains($worker, 'znews.js?v=24'), 'Service worker has a stale app URL.');
