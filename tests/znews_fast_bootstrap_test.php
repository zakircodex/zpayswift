...
fast_boot_expect(str_contains($bootstrap, "loadScript('/znews/assets/znews-creator.js?v=14')"), 'Creator module is unavailable after verified auth.');
...
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    fast_boot_expect(str_contains($worker, 'shell-v33'), 'Service-worker cache generation was not advanced.');
    fast_boot_expect(str_contains($worker, "SHELL_REVISION = 'adsterra-cross-origin-frame-v1'"), 'Adsterra cross-origin frame shell revision is missing.');
    ...
    fast_boot_expect(str_contains($worker, 'znews-bootstrap.js?v=36'), 'Service worker has a stale bootstrap URL.');
    fast_boot_expect(str_contains($worker, 'znews.js?v=30'), 'Service worker has a stale app URL.');
