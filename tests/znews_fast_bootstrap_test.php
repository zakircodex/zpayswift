...
fast_boot_expect(str_contains($bootstrap, "loadScript('/znews/assets/znews-creator.js?v=9')"), 'Creator module is unavailable after verified auth.');
...
foreach ([$embeddedWorker, $standaloneWorker] as $worker) {
    fast_boot_expect(str_contains($worker, 'shell-v22'), 'Service-worker cache generation was not advanced.');
    fast_boot_expect(str_contains($worker, "SHELL_REVISION = 'rich-text-my-posts-1'"), 'Rich-text/My Posts shell revision is missing.');
    fast_boot_expect(str_contains($worker, 'znews-bootstrap.js?v=27'), 'Service worker has a stale bootstrap URL.');
    fast_boot_expect(str_contains($worker, 'znews.js?v=23'), 'Service worker has a stale app URL.');
