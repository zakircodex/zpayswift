...
reader_expect(str_contains($index, 'znews-bootstrap.js?v=22'), 'Reload-safe reader bootstrap version is not activated.');
...
reader_expect(strpos($bootstrap, 'znews-reader.js?v=3') < strpos($bootstrap, 'znews.js?v=20'), 'Reader API capture must load before the main app.');
...
reader_expect(str_contains($serviceWorker, "const CACHE_NAME = 'zsky24-embedded-shell-v17'"), 'Mobile viewport PWA cache version is stale.');
...
reader_expect(str_contains($serviceWorker, 'znews-weekly-review.js?v=1'), 'Weekly creator report JS is missing from the shell cache.');
reader_expect(str_contains($serviceWorker, 'znews-bootstrap.js?v=22'), 'Reload-safe bootstrap is missing from the shell cache.');
