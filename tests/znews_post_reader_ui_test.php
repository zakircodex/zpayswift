...
reader_expect(str_contains($index, 'znews-bootstrap.js?v=14'), 'Latest reader bootstrap version is not activated.');
...
reader_expect(strpos($bootstrap, 'znews-reader.js?v=3') < strpos($bootstrap, 'znews.js?v=11'), 'Reader API capture must load before the main app.');
reader_expect(str_contains($bootstrap, 'znews-instant-comments.js?v=4'), 'Updated instant-comment module is not loaded.');

reader_expect(str_contains($serviceWorker, "const CACHE_NAME = 'zsky24-embedded-shell-v10'"), 'Mobile viewport PWA cache version is stale.');
...
reader_expect(str_contains($serviceWorker, 'znews-bootstrap.js?v=14'), 'Latest bootstrap is missing from the shell cache.');
