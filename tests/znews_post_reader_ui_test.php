...
reader_expect(str_contains($bootstrap, 'znews-reader.css?v=2'), 'Latest reader stylesheet is not activated after first paint.');
...
reader_expect(str_contains($index, 'znews-bootstrap.js?v=30'), 'Reload-safe reader bootstrap version is not activated.');
...
reader_expect(str_contains($serviceWorker, "const CACHE_NAME = 'zsky24-embedded-shell-v25'"), 'Mobile viewport PWA cache version is stale.');
...
reader_expect(str_contains($serviceWorker, 'znews-bootstrap.js?v=30'), 'Reload-safe bootstrap is missing from the shell cache.');
