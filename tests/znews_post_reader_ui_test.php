...
reader_expect(str_contains($index, 'znews-bootstrap.js?v=34'), 'Reload-safe reader bootstrap version is not activated.');
...
reader_expect(str_contains($serviceWorker, "const CACHE_NAME = 'zsky24-embedded-shell-v29'"), 'Mobile viewport PWA cache version is stale.');
...
reader_expect(str_contains($serviceWorker, 'znews-weekly-review.js?v=3'), 'Weekly creator report JS is missing from the shell cache.');
reader_expect(str_contains($serviceWorker, 'znews-bootstrap.js?v=34'), 'Reload-safe bootstrap is missing from the shell cache.');
