...
check(str_contains($index, 'znews-bootstrap.js?v=7'), 'Latest bootstrap is not activated');
...
check(strpos($bootstrap, 'znews-reader.js?v=3') < strpos($bootstrap, 'znews.js?v=4'), 'Reader capture must load before app');
...
check(str_contains($serviceWorker, "const CACHE_NAME = 'zsky24-embedded-shell-v2'"), 'Embedded reader shell cache is stale');
check(str_contains($serviceWorker, 'znews-bootstrap.js?v=7'), 'Latest bootstrap is missing from cache');
