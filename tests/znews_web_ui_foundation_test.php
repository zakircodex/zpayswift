...
check(str_contains($index, 'znews-bootstrap.js?v=17'), 'Latest bootstrap is not activated');
...
check(str_contains($app, 'state.openingPostId === postId'), 'Concurrent same-post opening guard is missing');
check(str_contains($app, 'state.viewStartingPostId === postId'), 'Single-flight view-start guard is missing');
...
check(strpos($bootstrap, 'znews-reader.js?v=3') < strpos($bootstrap, 'znews.js?v=16'), 'Reader capture must load before app');
