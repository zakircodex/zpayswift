...
birthday_test_expect(is_string($appClientSource) && str_contains($appClientSource, 'targetBytes = 600 * 1024') && str_contains($appClientSource, 'safeRawBytes = 680 * 1024') && str_contains($appClientSource, "type: 'image/jpeg'"), 'Large Android photos must fit the verified delivery fallback without cropping.');
