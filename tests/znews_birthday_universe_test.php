...
birthday_test_expect(is_string($sceneSource) && str_contains($sceneSource, 'positionCanvas') && str_contains($sceneSource, 'maximumOffset'), 'The WebGL canvas must remain bounded to the Universe instead of covering controls on Android.');
...
birthday_test_expect(is_string($birthdayCss) && str_contains($birthdayCss, '.universe-space-canvas{position:absolute') && str_contains($birthdayCss, '.public-share-panel{position:relative;z-index:7'), 'Share and QR controls must render above the bounded animated scene.');
...
birthday_test_expect(is_string($birthdayPublicSource) && strpos($birthdayPublicSource, 'class="public-share-panel"') < strpos($birthdayPublicSource, 'id="birthdayPublicAd"'), 'Public share and QR controls must appear before the advertisement.');
$templateClientSource = file_get_contents(dirname(__DIR__) . '/znews/birthday/assets/birthday-templates.js');
birthday_test_expect(is_string($templateClientSource) && str_contains($templateClientSource, 'moonSparkles') && str_contains($templateClientSource, 'master.gain.value = .44') && str_contains($templateClientSource, 'createConvolver'), 'Birthday rendering must include moon sparkles and the louder layered ambient soundtrack.');
birthday_test_expect(is_string($templateClientSource) && str_contains($templateClientSource, 'celebrationEffectsController') && str_contains($templateClientSource, 'schedulePop') && str_contains($templateClientSource, 'SpeechSynthesisUtterance'), 'Entering a Birthday Universe must trigger firework audio and a personalized spoken greeting.');
