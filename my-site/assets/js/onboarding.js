// My-Site onboarding demo. It stores tenant config in browser localStorage only.
(function initOnboarding() {
  ...
  function readForm() {
    ...
    const slug = window.MySiteDemoStore.slugify(siteName);
    ...
    return {
      ...
      commission: {
        topup_per_1000: Number(data.get('topup_per_1000') || 0),
        bundle_per_1000: Number(data.get('bundle_per_1000') || 0),
      ...
      features: {
        ...
        mfs: true,
        tracking: true,
        worker: data.get('worker_enabled') === 'true',
        telegram: data.get('telegram_enabled') === 'true'
      },
      telegram: {
        enabled: data.get('telegram_enabled') === 'true',
        bot_name: ''
      }
    };
  }
  ...
  function publishTenant(expired) {
    ...
    let tenant = tenantData.plan === 'SUBSCRIPTION'
      ? window.MySiteDemoStore.makePaidTenant(tenantData)
      : window.MySiteDemoStore.makeTrialTenant(tenantData);
  ...
  document.querySelector('[data-expire-demo]')?.addEventListener('click', function () {
    publishTenant(true);
    window.location.href = '../expired/index.html';
  });

  document.querySelector('[data-clear-demo]')?.addEventListener('click', function () {
    window.MySiteDemoStore.clear();
    alert('Demo tenant cleared.');
    updatePreview();
  });

  const existing = window.MySiteDemoStore.load();
  if (existing) {
